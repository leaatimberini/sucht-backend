...
export class PaymentsService {
  ...
  constructor(
    ...
    if (!accessToken) {
      this.logger.error('[Constructor] MERCADO_PAGO_ACCESS_TOKEN no está configurado en .env');
  ...
  async createPreference(
    ...
    try {
      ...
      return {
        ...
        this.logger.error('Error al crear la preferencia de pago en Mercado Pago', error.response?.data || error.message);
  ...
  public async handleWebhook(paymentData: { id: string }) {
    ...
    try {
      ...
      // No relanzar el error para que MP no reintente indefinidamente si es un error nuestro
    }
  }
}