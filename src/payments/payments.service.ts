...
export class PaymentsService {
  ...
  async createPreference(buyer: User, data: AcquireTicketDto & { promoterUsername?: string }) {
    ...
    const preferenceBody = {
      ...
      external_reference: JSON.stringify({
        type: 'TICKET_PURCHASE', // Se especifica el tipo de compra
  ...
  private async processApprovedPayment(paymentId: string) {
    ...
    } else { // Asumimos TICKET_PURCHASE por defecto
  ...
  public async handleWebhook(paymentData: { id: string }) {
    ...
    try {
        ...
        // Devolvemos una respuesta exitosa a Mercado Pago para que no siga reintentando
        // aunque haya habido un error interno.
    }
  }
